...
def borrow_book(book_id):
    ...
    db.execute("INSERT INTO borrowed_books (user_id, book_id) VALUES (?, ?)", (g.user["id"], book_id))
...
def return_book(book_id):
    db = get_db()
    borrowed = db.execute("SELECT * FROM borrowed_books WHERE user_id=? AND book_id=?",
                          (g.user["id"], book_id)).fetchone()
    if not borrowed:
